...
state integer not null DEFAULT 1, -- 0 삭제 1 정상 
reg_dt timestampz not null DEFAULT now(), 
CONSTRAINT note_pkey PRIMARY KEY (note_no),
CONSTRAINT users_user_no_fkey FOREIGN KEY (user_no) REFERENCES public.user(user_no) ON DELETE CASCADE ON UPDATE CASCADE,
);
